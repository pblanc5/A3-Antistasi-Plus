...
[2, format ["Sending %1 total troops to reinforce", _totalReinf], "reinforcementsAI.sqf"] call A3A_fnc_log;
...
while {_totalReinf > 0} do
{
	...
	if !("carrier" in _airport) then {
		...
		[2, format ["Airport %1 self-reinforced with %2 troops", _airport, _numTroops], "reinforcementsAI.sqf"] call A3A_fnc_log;
	...
	[2, format ["Reinforcing garrison %1 from %2 with %3 troops", _target, _airport, _numTroops], "reinforcementsAI.sqf"] call A3A_fnc_log;
